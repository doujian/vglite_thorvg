 ...
 *********************/
/* LVGL config removed */
#ifdef VG_LITE_RENDER_BACKEND

#include "vg_lite.h"
/* LVGL includes removed */
#include <thorvg.h>
#include <float.h>
#include <math.h>
#include <stdlib.h>
...
#if VG_LITE_YUV_SUPPORT
    #include <libyuv/convert_argb.h>
#endif

/* GL backend includes */
#if VG_LITE_RENDER_BACKEND == 2
    /* Use ThorVG's GL declarations which include all needed functions */
...
/* STB image write for PNG export - use extern, implementation in tests/stb_impl.cpp */
extern "C" {
...
/* VG_LITE_ALIGN macro is defined in vg_lite.h */
...
/* GL context check for Windows */
#if VG_LITE_RENDER_BACKEND == VG_LITE_RENDER_GL && defined(_WIN32)
...
extern "C" {
    ...
    vg_lite_error_t vg_lite_allocate(vg_lite_buffer_t * buffer)
    {
        ...
        size_t size = VG_LITE_ALIGN(buffer->height * stride, VG_LITE_BUF_ADDR_ALIGN);
#if defined(__ANDROID__)
        buffer->memory = memalign(VG_LITE_BUF_ADDR_ALIGN, size);
#elif defined(_WIN32)
        ...
        buffer->handle = nullptr;  // CPU-allocated buffers have no GPU handle
    ...
    vg_lite_error_t vg_lite_free(vg_lite_buffer_t * buffer)
    {
        assert(buffer->memory);
#if VG_LITE_RENDER_BACKEND == 2
        /* GL backend: delete GL buffer if handle was set by vg_lite_upload_buffer() */
        if (buffer->handle != nullptr) {
            GLuint gl_buffer = (GLuint)(uintptr_t)buffer->handle;
    ...
    vg_lite_error_t vg_lite_upload_buffer(vg_lite_buffer_t * buffer, vg_lite_uint8_t * data[3], vg_lite_uint32_t stride[3])
    {
        ...
        if (buffer == nullptr || data == nullptr || data[0] == nullptr) {
            return VG_LITE_INVALID_ARGUMENT;
        }
        ...
        /* Calculate data size: BGRA8888 = 4 bytes per pixel */
        size_t data_size = buffer->height * stride[0];
        
        /* Upload data to GPU */
        glBufferData(GL_PIXEL_UNPACK_BUFFER, data_size, data[0], GL_STATIC_DRAW);

        /* Unbind buffer */
        glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);

        /* Store GL buffer handle */
        buffer->handle = (vg_lite_pointer)(uintptr_t)gl_buffer;

        return VG_LITE_SUCCESS;
#else
        /* SW backend: not supported */
        (void)buffer;
        (void)data;
        (void)stride;
    ...
    vg_lite_error_t vg_lite_map(vg_lite_buffer_t * buffer, vg_lite_map_flag_t flag, int32_t fd)
    {
        ...
        if (flag == VG_LITE_MAP_USER_MEMORY) {
            /* For VG_LITE_MAP_USER_MEMORY, the buffer already has CPU-accessible memory.
             * This is the case when user allocated memory directly (not through vg_lite_allocate
             * or vg_lite_upload_buffer). The memory is already CPU-accessible.
             */
            ...
            /* DMABUF support requires platform-specific implementation
             * using EGL extensions (eglCreateImageKHR, etc.)
             * Not implemented in this version.
             */
            (void)fd; /* File descriptor for DMABUF */
            return VG_LITE_NOT_SUPPORT;
        }
        
        return VG_LITE_INVALID_ARGUMENT;
#else
        /* SW backend implementation */
        if (flag == VG_LITE_MAP_USER_MEMORY) {
            /* For SW backend, user memory is already CPU-accessible.
             * No special mapping needed - just validate the buffer.
             */
            if (buffer->memory == nullptr) {
                return VG_LITE_INVALID_ARGUMENT;
            }
        ...
        /* DMABUF not supported for SW backend */
        (void)fd;
        return VG_LITE_NOT_SUPPORT;
#endif
    }
...
#if VG_LITE_RENDER_BACKEND == VG_LITE_RENDER_GL
        /* GL backend implementation */
        /* For user memory mapping, unmap is a no-op since the memory
         * was never transferred to GPU ownership.
         */
        if (buffer->memory == nullptr) {
            return VG_LITE_INVALID_ARGUMENT;
        }
...
#else
        /* SW backend implementation */
        /* For SW backend, unmapping is a no-op since memory is always CPU-accessible */
        if (buffer->memory == nullptr) {
            return VG_LITE_INVALID_ARGUMENT;
        }
        return VG_LITE_SUCCESS;
...
#if VG_LITE_RENDER_BACKEND == VG_LITE_RENDER_GL
        /* For GL backend with ThorVG:
         * ThorVG's GlCanvas manages GPU resources internally and handles
         * CPU-GPU synchronization during canvas->sync() (called in vg_lite_finish()).
         * 
         * The vg_lite_flush_mapped_buffer() API is provided for explicit synchronization
         * when the user wants to ensure CPU writes are visible to GPU before rendering.
         * Since ThorVG manages its own GL state and buffer mappings, we perform a
         * lightweight synchronization here.
         * 
         * Note: For buffers mapped with explicit flush semantics, the caller is
         * responsible for ensuring proper usage patterns. ThorVG's rendering
         * pipeline will handle the actual GPU upload during draw operations.
         */
        
        /* For now, return success - ThorVG handles synchronization internally.
         * The actual GPU buffer updates occur during canvas->sync() when
         * vg_lite_finish() is called.
         */
        return VG_LITE_SUCCESS;
#else
        /* SW backend: no GPU-CPU sync needed, data is already in CPU memory */
        return VG_LITE_SUCCESS;
#endif
    }
                                      ...
                                      vg_lite_filter_t filter)
    {
        ...
        TVG_CHECK_RETURN_VG_ERROR(picture->blend(blend_method_conv(blend)));
        TVG_CHECK_RETURN_VG_ERROR(picture->clip(shape));
        TVG_CHECK_RETURN_VG_ERROR(ctx->canvas->add(picture));
    ...
    {
        (void)tessellation_width;
        (void)tessellation_height;
#if VG_LITE_THREAD_RENDER
        /* Threads Count */
        ...
        /* Reset the context canvas to ensure clean state */
        auto ctx = vg_lite_ctx::get_instance();
#if VG_LITE_RENDER_BACKEND == VG_LITE_RENDER_GL
        ctx->canvas.reset(GlCanvas::gen());
#else
        ctx->canvas.reset(SwCanvas::gen());
#endif
    ...
    {
        /* Clear the context canvas before terminating */
        auto ctx = vg_lite_ctx::get_instance();
        ctx->canvas.reset();
        ctx->tvg_target_buffer = nullptr;
                                 ...
                                 vg_lite_color_t color)
    {
        ...
        /**
         * Special handling for DST_IN blend mode using ThorVG's MaskMethod::InvAlpha.
         *   - The mask shape is filled with (255 - A(color)), used with InvAlpha.
         *   - InvAlpha formula: dst = dst * (255 - mask_alpha) / 255.
         *   - When color=0, mask alpha=255 inside the shape, so (255-255=0): inside is cleared; outside (255-0=255): preserved.
         *   - The target buffer is cleared with memset before rendering, ensuring inside pixels are zeroed.
        ...
        if(blend == VG_LITE_BLEND_DST_IN) {
             ...
             */
            memset(target->memory, 0, target->stride * target->height);
                                       ...
                                       vg_lite_color_t color)
    {
        ...
        if(!path->stroke) {
            path->stroke = (vg_lite_stroke_t *)calloc(1, sizeof(vg_lite_stroke_t));
...
#if VG_LITE_LINEAR_GRADIENT_EXT_SUPPORT
            case gcFEATURE_BIT_VG_LINEAR_GRADIENT_EXT:
#endif

#if VG_LITE_16PIXELS_ALIGN
                                            ...
                                            vg_lite_uint8_t pre_multiplied)
    {
        ...
        if(!count || count > VG_LITE_MAX_COLOR_RAMP_STOPS || color_ramp == NULL)
        ...
        for(
            ...
            (src_ramp < src_ramp_last) && (trg_count < VG_LITE_MAX_COLOR_RAMP_STOPS + 2);
            src_ramp += 1) {
...
Empty_sequence_handler:
                                             ...
                                             vg_lite_filter_t filter)
    {
        ...
        tvg::Fill::ColorStop colorStops[VG_LITE_MAX_COLOR_RAMP_STOPS];
                                            ...
                                            vg_lite_uint8_t pre_multiplied)
    {
        ...
        for(
            ...
            (srcRamp < srcRampLast) && (trgCount < VG_LITE_MAX_COLOR_RAMP_STOPS + 2);
            srcRamp += 1) {
            /* Must be in increasing order. */
...
Empty_sequence_handler:
                                     ...
                                     vg_lite_uint32_t * stops)
    {
        ...
        if(!count || count > VG_LITE_MAX_GRADIENT_STOPS || colors == NULL || stops == NULL)
            return VG_LITE_SUCCESS;
                                      ...
                                      vg_lite_blend_t blend)
    {
        ...
        tvg::Fill::ColorStop colorStops[VG_LITE_MAX_GRADIENT_STOPS];
                                             ...
                                             vg_lite_filter_t filter)
    {
        ...
        TVG_CHECK_RETURN_VG_ERROR(radialGrad->radial(grad->radial_grad.cx, grad->radial_grad.cy, grad->radial_grad.r, grad->radial_grad.fx, grad->radial_grad.fy, 0.0f));
                                                                ...
                                                                vg_lite_uint32_t background_segment_size)
    {
        (void)stream_id;
        (void)buffer;
        (void)background_segment_count;
        (void)background_segment_size;
        return VG_LITE_NOT_SUPPORT;
    }
    ...
    {
        printf("VG_LITE: command:\n");
        printf("VG_LITE: @[commit]\n");
        return VG_LITE_SUCCESS;
    }
} /* extern "C" */
...
static uint8_t vlc_op_arg_len(uint8_t vlc_op)
{
    switch(vlc_op) {
        ...
        default:
            fprintf(stderr, "VG_LITE: UNKNOWN_VLC_OP: 0x%x\n", vlc_op);
            assert(false);
            break;
    }
...
static Result shape_set_stroke(Shape * shape, const vg_lite_path_t * path)
{
    switch(path->path_type) {
        ...
        default:
            fprintf(stderr, "VG_LITE: unknown path type: %d\n", path->path_type);
    ...
    if(path->stroke->pattern_count) {
        assert(path->stroke->dash_pattern);
...
static Result shape_append_path(Shape * shape, vg_lite_path_t * path, vg_lite_matrix_t * matrix)
    ...
    TVG_CHECK_RETURN_RESULT(shape->clip(cilp));
...
static Result canvas_set_target(vg_lite_ctx * ctx, vg_lite_buffer_t * target)
    ...
    if(TVG_IS_VG_FMT_SUPPORT(target->format)) {
        ...
        assert(target->stride >= target->width);
        assert(VG_LITE_IS_ALIGNED(target->stride, sizeof(uint32_t)));
    ...
    TVG_CHECK_RETURN_RESULT(sw_canvas->target(
                           ...
                           vg_lite_color_t color)
{
    ...
    assert(VG_LITE_IS_ALIGNED(source->memory, 8));
    ...
    TVG_CHECK_RETURN_RESULT(picture->load((uint32_t *)image_buffer, source->width, source->height, tvg::ColorSpace::ARGB8888, true));
...
static void vg_lite_matrix_multiply(vg_lite_matrix_t * matrix, const vg_lite_matrix_t * mult)
{
    ...
    memcpy(matrix->m, &temp.m, sizeof(temp.m));
}
...
vg_lite_error_t vg_lite_dump_buffer(vg_lite_buffer_t * buffer, const char * filename)
{
    /* Parameter validation */
    ...
    /* Check if buffer has valid memory */
    if (buffer->memory == nullptr) {
        return VG_LITE_INVALID_ARGUMENT;
    }
    ...
    switch (buffer->format) {
        ...
        case VG_LITE_L8:
            break;
        default:
            fprintf(stderr, "VG_LITE: dump_buffer unsupported format: %d\n", buffer->format);
            return VG_LITE_NOT_SUPPORT;
    }

    vg_lite_uint32_t width = buffer->width;
    vg_lite_uint32_t height = buffer->height;
    vg_lite_uint32_t stride = buffer->stride;

    /* Allocate temporary buffer for RGBA output */
    vg_lite_uint32_t *temp_buffer = (vg_lite_uint32_t *)malloc(width * height * 4);
    if (temp_buffer == nullptr) {
        return VG_LITE_OUT_OF_RESOURCES;
    }

    /* Create a temporary vg_lite_buffer_t for conversion */
    vg_lite_buffer_t temp_buf;
    memset(&temp_buf, 0, sizeof(temp_buf));
    temp_buf.width = width;
    temp_buf.height = height;
    temp_buf.stride = width * 4;  /* RGBA stride */
    temp_buf.format = VG_LITE_BGRA8888;
    temp_buf.memory = temp_buffer;

    /* Create source buffer descriptor */
    vg_lite_buffer_t src_buf;
    memset(&src_buf, 0, sizeof(src_buf));
    src_buf.width = width;
    src_buf.height = height;
    src_buf.stride = stride;
    src_buf.format = buffer->format;
    src_buf.memory = buffer->memory;

    /* Convert source format to BGRA8888 */
    switch (buffer->format) {
        case VG_LITE_BGRA8888:
            conv_bgra8888_to_bgra8888.convert(&temp_buf, &src_buf);
            break;
        case VG_LITE_RGBA8888: {
            /* RGBA8888 -> BGRA8888 requires swapping R and B */
            uint8_t *src = (uint8_t *)buffer->memory;
            uint8_t *dst = (uint8_t *)temp_buffer;
            for (vg_lite_uint32_t y = 0; y < height; y++) {
                for (vg_lite_uint32_t x = 0; x < width; x++) {
                    uint32_t offset = y * stride + x * 4;
                    uint32_t dst_offset = y * temp_buf.stride + x * 4;
                    dst[dst_offset + 0] = src[offset + 2];  /* B <- R */
                    dst[dst_offset + 1] = src[offset + 1];  /* G */
                    dst[dst_offset + 2] = src[offset + 0];  /* R <- B */
                    dst[dst_offset + 3] = src[offset + 3];  /* A */
                }
            }
            break;
        }
        case VG_LITE_RGB565:
            conv_bgr565_to_bgra8888.convert(&temp_buf, &src_buf);
            break;
        case VG_LITE_L8:
            conv_l8_to_bgra8888.convert(&temp_buf, &src_buf);
            break;
        default:
            free(temp_buffer);
            return VG_LITE_NOT_SUPPORT;
    ...
    /* Swap R and B channels to convert BGRA to RGBA for PNG */
    uint8_t *rgba_data = (uint8_t *)temp_buffer;
    for (vg_lite_uint32_t y = 0; y < height; y++) {
        for (vg_lite_uint32_t x = 0; x < width; x++) {
            uint32_t offset = y * temp_buf.stride + x * 4;
            uint8_t temp = rgba_data[offset + 0];  /* B */
            rgba_data[offset + 0] = rgba_data[offset + 2];  /* R <- B */
            rgba_data[offset + 2] = temp;  /* B <- original R */
        }
    }

    /* Write PNG file */
    int result = stbi_write_png(filename, width, height, 4, temp_buffer, temp_buf.stride);

    free(temp_buffer);

    if (result == 0) {
        fprintf(stderr, "VG_LITE: failed to write PNG file: %s\n", filename);
        return VG_LITE_INVALID_ARGUMENT;
    }
